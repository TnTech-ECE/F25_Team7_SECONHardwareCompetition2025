...
These functions are explained in greater detail in their respective design documents [2] [4] [5]. The global controller shall also run the program that will dictate the Robot's behavior during it's operation. The robot's operation is illistrated in the figures 7-10. Once the start signal is given, whether that be the autonomous or manual start signal, a three minute timer will begin. This timer represents the three minutes the robot has to complete its operation. Once the timer begins, the UAV will launch and begin its operation as described in the object detection design document [2]. Using the data collected by the UAV and robot mounted camera, objects on the gameboard will be classified. Once either an astro duck or antenna tower is identified, a task is created and entered into an event queue to allow the robot to complete each task one at a time. Once the internal timer has thirty seconds remaining, the robot will complete its active task before returning to its starting zone. The UAV will then transmitt the LED color data to Earth before landing.

In order to meet specifications 1, 5, and 6; a small single board computer is neccessary for this implementation, because of the limited size constraints of the robot. The robot shall use a Jetson Orin, more specifically a Jetson Orin Nano Super Developer Kit. The robot shall use a Jeston Orin instead of the Jetson Nano or the Jetson TX2 because of its processing power. The jetson Orin contains a 6-core ARM Cortex 64-bit CPU that operates at 1.7 GHz, a 1024-core NVIDIA Ampere architecture GPU that operates at 1020MHz, and 8GB 128-bit LPDDR5 RAM [3]. This makes the Jetson Orin more powerful than the Nano. The Jetson TX2's CPU and GPU both operate at a higher frequency then the Orin [6], however, the Orin's CPU and GPU cantain signifigantly more cores then the TX2. The Jetson Orin's component processing power will allow it to execute its program function quickly and effeciently. The Jetson Orin shall also use Unbuntu as it's operating system. This will allow for the ROS2 NAV2 stack to be implemented efficiently. The WiFi adaptor that will be used to connect to the UAV mounted camera to the global controller is Unbuntu compatible [6].

The Jetson series of single board computers are built with processing AI loads in mind. The robot shall be using two object detection algorithms along side an object detection post processing algorithm, so having a computer that is designed for effectively handling that type of work load is desireable. The Jetson Orin is listed as having an AI performance of 67 tera-operation per second (TOPS) [2]. The Nano and the TX2 have an AI performance of 0.472 and 1.33 tera floating point operation per second (TFLOPS) respectively [7] [8]. TOPS and TFLOPS are both metrics that are used to describe AI performance [9] [10], though they are different, with TOPS referring to 8 bit integer operations while TFLOPS refers to floating point operation [9]. However, a general case conversion can be made to compare the two [11]. An exact conversion is hardware architecture dependent, however, using the general case the Orin is shown to be the most powerful of the three listed computers. The previous competition’s robot was able to utilize the Jetson Nano, however, the 2026 competition’s robot will need to handle more AI loads than the previous competition’s robot. Thus the more powerful Jetson Orin shall allow the robot the ability to process all of its AI loads quickly, allowing the robot to complete all of its required task within the 3 minute time limit. The Jetson Nano will serve as a potential replacement global controller in case it is needed.
